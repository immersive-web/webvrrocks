...
(function(){
	...
	internal.smartLoad = function(html, options) {
		...
		var elWithPagePath = html.querySelector('[data-current-path]');
		if (elWithPagePath) {
			var pagePath = elWithPagePath.getAttribute('data-current-path');
			if (pagePath) {
				elWithPagePath = document.querySelector('[data-current-path]');
				if (elWithPagePath) {
					elWithPagePath.setAttribute('data-current-path', pagePath);
				}
			}
		}
	...
	internal.updateContent = function(html, options){
		...
		if(options.smartLoad) tmp = internal.smartLoad(tmp, options, html);
